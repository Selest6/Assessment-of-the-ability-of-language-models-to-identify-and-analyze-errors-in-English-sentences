...
headers = ['label', 'category', 'explanation']
...
for sentence in tqdm(sentences):
    ...
    explanation = "Nothing"
    category = "Nothing"
    while label == "Nothing" or explanation == "Nothing" or category == "Nothing":
        try:
            time.sleep(8)
            ...
            label_match = re.search(r'Label:.+(\d+).+Category:', data)
            category_match = re.search(r'Category:(.+?)Explanation:', data)
            explanation_match = re.search(r'Explanation:(.+)', data)
    ...
    with open(filename, "a", encoding="utf-8") as file:
        file.write(f"{label}\t{category}\t{explanation}\n")
